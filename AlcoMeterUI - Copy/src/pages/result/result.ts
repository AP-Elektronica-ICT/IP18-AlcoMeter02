...
export class ResultPage {
  ...
  genGetal(){
    var getal = (Math.round(Math.random() *100)/100);
    this.fireBaseProvider.addItem(this.auth.getLoggedUID(),15,'Male','Belgium',getal);
    return getal;
  }
  
  ngOnInit(){
    this.promilleMeting = this.genGetal();
    var testget = this.promilleLevel;
    ...
    getMaxPro(land){
        ...
        for(i =0;i<this.array.length;i++){
            if(this.array[i][0] == land){
              ...
              this.newChart(this.promilleMeting,this.maxPro); // Hier waarde van meting invoegen !!Hier waarde van meting invoegen !!Hier waarde van meting invoegen !!Hier waarde van meting invoegen !!Hier waarde van meting invoegen !!Hier waarde van meting invoegen !!
